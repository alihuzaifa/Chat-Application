...
const friendsGet = () => {
    ...
    unsubscribe = onSnapshot(q, (querySnapshot) => {
        ...
        querySnapshot.forEach((doc) => {
            let data = doc.data()
            list.innerHTML += `<div class="col-12 mb-2 d-flex py-2 rounded bg-dark text-white" onclick = "selectedUser('${data.uid}')">
            <img src="${data.image}" class="img-fluid friend-image friend" alt="person" onclick = "selectedUser('${data.uid}')">
            <h5 class="mt-3 ms-2 friend-name" onclick = "selectedUser('${data.uid}')">${data.userName}</h5>
        </div>`
        });
    });
    userImage()
